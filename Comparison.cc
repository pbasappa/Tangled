...
void OrderMaker :: addAtt(int index, int attIndex)
{
	whichAtts[index] = attIndex;
...
void OrderMaker :: addType(int index, Type type)
{
	whichTypes[index] = type;

}
	
void OrderMaker :: Write (char *metadata, int l) 
{
	ofstream writer;
	writer.open(metadata);
	writer<<"sorted\n";
	
	//Writing the run length into metadata file
	writer<<l<<"\n";
	
	//Writing number of attributes
	writer<<numAtts<<"\n";
	
	//Writing the attribute data types into metadata file
	for (int i = 0; i < numAtts; i++)
	{
		writer<<whichAtts[i]<<"\n";
		if (whichTypes[i] == Int)
...
void CNF :: getAttributes(int **att,int **litAttr,int **num)
{
	*att = new int[numAnds];
	*num = new int;
	*litAttr = new int[numAnds];
	int counter = 0;
	...
	for( int i = 0; i < numAnds; i++)
	{
		for (int j = 0; j < orLens[i]; j++) {
			...
			if(orList[i][j].op == Equals)
			{
				...
				if((orList[i][j].operand1 == Literal && (orList[i][j].operand2 == Left || orList[i][j].operand2 == Right)) ||
				   (orList[i][j].operand2 == Literal && (orList[i][j].operand1== Left || orList[i][j].operand1 == Right)))
				{
			
					if(orList[i][j].operand1 == Literal)
					{
						(*litAttr)[counter] = orList[i][j].whichAtt1;
						(*att)[counter++] = orList[i][j].whichAtt2;					
					}
					else
					{
						(*litAttr)[counter] = orList[i][j].whichAtt2;
						(*att)[counter++] = orList[i][j].whichAtt1;
					}
				}
			}			
